...
string = input('Введите стих. Он состоят из фраз. Фразы разделяются пробелами. Фраза может состоять из одного слова, если во фразе несколько слов, то они разделяются дефисам: ').split()

list = [sum(x in 'аеёиоуыэюя' for x in line) for line in string]
